# This code is not orignally mine. This has been tweeked from:
...
class Paint(object):
    ...
    def __init__(self):
        ...
        self.save = Button(self.root, text='save', command=self.save_drawing)
        self.save.grid(row=0, column=5)
    ...
    def save_drawing(self):
        x = self.root.winfo_rootx()+130
        y = self.root.winfo_rooty()+90
        self.img = ImageGrab.grab(bbox=(x,y,x+740,y+750)).convert('L')
        self.img = 255-np.array(self.img.resize((28, 28), Image.ANTIALIAS))
        self.root.destroy()
